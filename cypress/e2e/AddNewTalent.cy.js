   ...
   cy.restoreLocalStorage()
    ...
    it('Add new talent flow', function () {
        ...
        cy.wait('@countriesInfo').then((intercept) => {
         ...
         talentOnboard.endDate().click({force:true}).clear().type(generatedTalentData.endDate)
         talentOnboard.eligibleReimbursements().then(($el) => {
            let test = $el.prop('checked')
            test == generatedTalentData.eligibleReimbursements? null : talentOnboard.eligibleReimbursements().click({ force: true })
         })
         talentOnboard.workFromHome().then(($el) => {
            let test = $el.prop('checked')
            test == generatedTalentData.workFromHome? null : talentOnboard.workFromHome().click({ force: true })
         })
         ...
         talentOnboard.probationPeriodEnablement().then(($el) => {
            let test = $el.prop('checked')
            test == generatedTalentData.probationPeriodEnablement? null : talentOnboard.probationPeriodEnablement().click({ force: true })
         })
         ...
         generatedTalentData.esopProvided?talentOnboard.esopProvidedAcknowledge().click({force:true}):null
         ...
         talentOnboard.annualBonusEnablement().then(($el) => {
            let test = $el.prop('checked')
            test == generatedTalentData.annualBonusEnablement? null : talentOnboard.annualBonusEnablement().click({ force: true })
         })
